...
import static com.example.samplestickerapp.StickerContentProvider.VERSION_NUMBER_OF_IMAGE_DATA;
...
public class StickerPackLoader {
    ...
    private static ArrayList<StickerPack> fetchFromContentProvider(Cursor cursor) {
        ...
        do {
            ...
            final int versionNumberOfImageData = cursor.getInt(cursor.getColumnIndexOrThrow(VERSION_NUMBER_OF_IMAGE_DATA));
            final boolean avoidCache = cursor.getShort(cursor.getColumnIndexOrThrow(AVOID_CACHE)) > 0;
            final StickerPack stickerPack = new StickerPack(identifier, name, publisher, trayImage, publisherEmail, publisherWebsite, privacyPolicyWebsite, licenseAgreementWebsite, versionNumberOfImageData, avoidCache);
